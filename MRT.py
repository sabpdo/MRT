...
expInfo = {'participant': '', 'session': '001', 'age': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sabrinado/Library/CELSYS/MRT.py',
...
image = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=512, interpolate=True, depth=0.0)
indicatorQuestion1 = visual.TextStim(win=win, name='indicatorQuestion1',
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('trial123.xlsx'),
...
for thisTrial in trials:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    Question1Components = [image, indicatorQuestion1, responseQuestion1, key_resp]
    ...
    while continueRoutine:
        ...
        # *indicatorQuestion1* updates
        if indicatorQuestion1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            indicatorQuestion1.frameNStart = frameN  # exact frame index
            indicatorQuestion1.tStart = t  # local t and not account for scr refresh
            indicatorQuestion1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(indicatorQuestion1, 'tStartRefresh')  # time at next scr refresh
            indicatorQuestion1.setAutoDraw(True)
    ...
    trials.addData('indicatorQuestion1.started', indicatorQuestion1.tStartRefresh)
    trials.addData('indicatorQuestion1.stopped', indicatorQuestion1.tStopRefresh)
    # check responses
...
trials_2 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('part2Questions.xlsx'),
